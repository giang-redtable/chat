      ...
      if(!(data.msg==1 || data.msg==2 || data.msg==3 || data.msg==4)) {
        ...
        if(sampleData.length > 0) { //Sample data 남아있을 경우 계속 보여주면서  N일 경우 해당code pop
          ...
          //첫 추천은 API call 50개의 추천 메뉴정보 가져옴
            ...
            if (msg == "y") {
              if (preData.length == 0) {
                io.sockets.in('channel'+ data.channelId).emit('recom idx', '');
                io.sockets.in('channel'+ data.channelId).emit('init set', '');
                io.sockets.in('channel'+ data.channelId).emit('base cd', '');
                io.sockets.in('channel'+ data.channelId).emit('sample select', '');

                msgRe = '1 (한식) 또는 2 (中餐)를 입력해 주세요.';
                apiResponse(msgRe);
              } else {
                //메뉴 선택
                var url = "https://api.redtable.global/v3/restaurant_partner.php?city=seoul&search=" + preData[0].name;
              ...
              if (preData.length > 0) {
                ...
                retValue += '<a class="ui button blue">Yes</a> or <a class="ui button red">No</a>?';
                io.sockets.in('channel'+ data.channelId).emit('pre data', '');
                apiResponse(retValue);
              } else {
                //다음 메뉴
                ...
                var req = request({
                  ...
                  if (!error && response.statusCode === 200) {
                    ...
                    io.sockets.in('channel'+ data.channelId).emit('pre data', body.result);
                    io.sockets.in('channel'+ data.channelId).emit('recom idx', Number(data.recomIdx) + 1);
                    apiResponse(retValue);
                  }
                })
              }
            } else {
              var arrData = preData[1];
              preData.shift();
